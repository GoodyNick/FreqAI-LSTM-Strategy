...
class LSTMStrategy_v32(IStrategy):
    """
    ...
    use_confidence_filter_entry = CategoricalParameter([True, False], default=True, space="buy", load=True, optimize=True)
    ...
    use_trend_filter = CategoricalParameter([True, False], default=True, space="buy", load=True, optimize=True)
    ...
    use_target_exit_filter = CategoricalParameter([True, False], default=True, space="sell", load=True, optimize=True)
    use_trend_exit_filter = CategoricalParameter([True, False], default=True, space="sell", load=True, optimize=True)
    use_confidence_filter = CategoricalParameter([True, False], default=True, space="sell", load=True, optimize=True)
    use_weak_exit = CategoricalParameter([True, False], default=True, space="sell", load=True, optimize=True)
    use_volatility_exit_filter = CategoricalParameter([True, False], default=True, space="sell", load=True, optimize=True)
    ...
    buy_params = {
        "confidence_threshold_multiplier": 0.92065,
        "dynamic_long_threshold_multiplier": 1.03094,
        "dynamic_short_threshold_multiplier": 0.9857,
        "rolling_trend_threshold_multiplier": 0.90927,
        "stake_scaling_factor": 0.81246,
    ...
    sell_params = {
        "atr_stoploss_multiplier": 1.14273,
        "base_risk": 0.07157,
        "dynamic_long_exit_threshold_multiplier": 1.18834,
        "dynamic_short_exit_threshold_multiplier": 1.03154,
        "exit_trend_threshold_multiplier": 0.5315,
        "historical_volatility_factor": 0.69458,
        "max_risk_per_trade_multiplier": 0.03393,
        "min_profit_for_trailing": 0.03695,
        "min_trade_duration": 7,
        "prediction_confidence_factor": 0.69336,
        "soft_stoploss_pct": -0.2999,
        "timed_exit_long_threshold": 17,
        "timed_exit_short_threshold": 28,
        "use_confidence_filter": False,
        "use_target_exit_filter": False,
        "use_trend_exit_filter": False,
        "use_volatility_exit_filter": False,
        "use_weak_exit": False,
    }
    ...
    def populate_entry_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        df.loc[fallback_long & (df["enter_long"] == 0), ["enter_long", "enter_tag"]] = (1, "long_fallback")
        df.loc[fallback_short & (df["enter_short"] == 0), ["enter_short", "enter_tag"]] = (1, "short_fallback")

        return df

    def populate_exit_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        df["timed_exit_long"] = (
            df["rolling_trend"].rolling(self.timed_exit_long_threshold.value).max() > 0.015
        ).astype(int)

        df["timed_exit_short"] = (
            df["rolling_trend"].rolling(self.timed_exit_short_threshold.value).min() < -0.015
        ).astype(int)

        # ─── Trend Reversal Exits ─────────────────────────────
        if df["rolling_trend_scaled"].isnull().any() or df["exit_trend_threshold_base"].isnull().any():
            trend_exit_long = pd.Series(False, index=df.index)
            trend_exit_short = pd.Series(False, index=df.index)
            logger.warning("[WARN] NaNs in trend reversal inputs, skipping trend_exit_*")
        else:
            threshold_trend = df["exit_trend_threshold_base"] * self.exit_trend_threshold_multiplier.value
            trend_exit_long = df["rolling_trend_scaled"] < threshold_trend
            trend_exit_short = df["rolling_trend_scaled"] > threshold_trend

        # ─── Confidence Exits ────────────────────────────────
        confidence_exit_long = (
            df["prediction_confidence"].fillna(0) <
            df["confidence_threshold_base"].fillna(0) * self.confidence_threshold_multiplier.value
        )
        confidence_exit_short = confidence_exit_long

        # ─── Target Reversal Exits ───────────────────────────
        if df["&-s_target"].isnull().any() or df["dynamic_exit_threshold_base"].isnull().any():
            target_exit_long = pd.Series(False, index=df.index)
            target_exit_short = pd.Series(False, index=df.index)
            logger.warning("[WARN] NaNs in target exit inputs, skipping target_exit_*")
        else:
            threshold_exit_long = df["dynamic_exit_threshold_base"] * self.dynamic_long_exit_threshold_multiplier.value
            threshold_exit_short = df["dynamic_exit_threshold_base"] * self.dynamic_short_exit_threshold_multiplier.value
            target_exit_long = df["&-s_target"] < threshold_exit_long
            target_exit_short = df["&-s_target"] > threshold_exit_short

        # ─── Volatility Exits ────────────────────────────────
        vol_exit_long = df["vol_rank"] > df["vol_rank"].rolling(100).quantile(0.90)
        vol_exit_short = vol_exit_long
        ...
        if self.use_target_exit_filter.value:
            long_exit_conditions.append(target_exit_long)
            short_exit_conditions.append(target_exit_short)

        if self.use_trend_exit_filter.value:
            long_exit_conditions.append(trend_exit_long)
            short_exit_conditions.append(trend_exit_short)

        if self.use_confidence_filter.value:
            long_exit_conditions.append(confidence_exit_long)
            short_exit_conditions.append(confidence_exit_short)

        if self.use_volatility_exit_filter.value:
            long_exit_conditions.append(vol_exit_long)
            short_exit_conditions.append(vol_exit_short)

        if df["do_predict"].isnull().any():
            df["do_predict"] = 0
            strong_exit_long = (df["do_predict"] >= 0) & reduce(operator.or_, long_exit_conditions, False)
            strong_exit_short = (df["do_predict"] >= 0) & reduce(operator.or_, short_exit_conditions, False)
        else:
            strong_exit_long = (df["do_predict"] >= 0) & reduce(operator.or_, long_exit_conditions, False)
            strong_exit_short = (df["do_predict"] >= 0) & reduce(operator.or_, short_exit_conditions, False)

        df.loc[strong_exit_long, ["exit_long", "exit_tag"]] = (1, "strong_exit_long")
        df.loc[strong_exit_short, ["exit_short", "exit_tag"]] = (1, "strong_exit_short")

        # ─── Weak Exit Fallbacks ─────────────────────────────
        if self.use_weak_exit.value:
            df.loc[df["timed_exit_long"] == 1, ["exit_long", "exit_tag"]] = (1, "weak_exit_long")
            df.loc[df["timed_exit_short"] == 1, ["exit_short", "exit_tag"]] = (1, "weak_exit_short")

        profit_lock_long = (df["&-s_target"] < 0) & (df["&-s_target_mean"] < 0)
        profit_lock_short = (df["&-s_target"] > 0) & (df["&-s_target_mean"] > 0)

        if self.use_weak_exit.value:
            df.loc[profit_lock_long, ["exit_long", "exit_tag"]] = (1, "profit_lock_long")
            df.loc[profit_lock_short, ["exit_short", "exit_tag"]] = (1, "profit_lock_short")
