...
class LSTMStrategy_v44_5m(IStrategy):
    """
    ...
    buy_params = {
        "confidence_influence": 0.52,
        "dynamic_long_threshold_multiplier": 1.3,
        "dynamic_short_threshold_multiplier": 1.99,
        "prediction_smoothing_window": 43,
        "vol_rank_quantile": 0.44,
        "volatility_influence": 0.52,
    ...
    sell_params = {
        "atr_stoploss_multiplier": 7.6,
        "exit_long_threshold_multiplier": 1.15,
        "exit_short_threshold_multiplier": 1.18,
        "hard_max_stoploss_pct": -0.13,
        "historical_volatility_factor": 0.77,
        "prediction_confidence_factor": 0.47,
        "timed_exit_duration": 36,
    ...
    def save_prediction_metrics(self, filename="pred_metrics.csv"):
        ...
        if not LSTMStrategy_v44_5m.pred_metrics_storage:
            logger.warning("⚠️ No prediction metrics found to save.")
            return

        try:
            # Use class attribute directly
            df = pd.DataFrame(LSTMStrategy_v44_5m.pred_metrics_storage)
