...
class LSTMStrategy_v30(IStrategy):
    """
    ...
    confidence_threshold_multiplier = RealParameter(0.3, 1.2, default=1.0, space="buy", load=True, optimize=True)
    ...
    rolling_trend_threshold_multiplier = RealParameter(0.4, 2.0, default=1.1, space="buy", load=True, optimize=True)
    ...
    soft_stoploss_pct = RealParameter(-0.25, -0.02, default=-0.10, space="sell", load=True, optimize=True)
    ...
    buy_params = {
        "confidence_threshold_multiplier": 0.60761,
        "dynamic_long_threshold_multiplier": 0.77888,
        "dynamic_short_threshold_multiplier": 0.75765,
        "rolling_trend_threshold_multiplier": 1.84835,
        "stake_scaling_factor": 0.61561,
        "use_trend_filter": False,
    }

    # Sell hyperspace params:
    sell_params = {
        "atr_stoploss_multiplier": 1.68311,
        "base_risk": 0.09581,
        "dynamic_long_exit_threshold_multiplier": 1.17631,
        "dynamic_short_exit_threshold_multiplier": 0.59049,
        "exit_trend_threshold_multiplier": 0.30962,
        "historical_volatility_factor": 1.08726,
        "max_risk_per_trade_multiplier": 0.01525,
        "min_profit_for_trailing": 0.02046,
        "min_trade_duration": 7,
        "prediction_confidence_factor": 0.33594,
        "soft_stoploss_pct": -0.06555,
        "timed_exit_long_threshold": 38,
    ...
    def populate_exit_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        # ✅ Timed Exit Logic (can be toggled as weak exit)
        df["timed_exit_long"] = (
        ...
        if self.use_target_exit_filter.value:
            long_conditions.append(
                qtpylib.crossed_below(
                    df["&-s_target"],
                    df["dynamic_exit_threshold_base"] * self.dynamic_long_exit_threshold_multiplier.value
                )
            )
        ...
        if self.use_trend_exit_filter.value:
            long_conditions.append(
                qtpylib.crossed_below(
                    df["rolling_trend_scaled"],
                    df["exit_trend_threshold_base"] * self.exit_trend_threshold_multiplier.value
                )
            )
        ...
        strong_exit_long_conditions = (df["do_predict"] >= 0) & reduce(operator.or_, long_conditions) if long_conditions else pd.Series(False, index=df.index)
        ...
        if self.use_target_exit_filter.value:
            short_conditions.append(
                qtpylib.crossed_above(
                    df["&-s_target"],
                    df["dynamic_exit_threshold_base"] * self.dynamic_short_exit_threshold_multiplier.value
                )
            )
        ...
        strong_exit_short_conditions = (df["do_predict"] >= 0) & reduce(operator.or_, short_conditions) if short_conditions else pd.Series(False, index=df.index)
