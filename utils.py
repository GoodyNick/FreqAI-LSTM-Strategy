...
def plot_feature_importance(
    ...
    Supports PyTorch, CatBoost, LightGBM, and XGBoost models.

    :param model: Any = The trained model.
    :param pair: str = Trading pair (e.g., BTC/USD).
    :param dk: FreqaiDataKitchen = Data container for current coin/loop.
    :param count_max: int = Number of features to plot (top and bottom).
    """
    from freqtrade.plot.plotting import go, make_subplots, store_plot_file

    models = {}
    
    # ✅ If Multi-Output Model, Extract Individual Estimators
    ...
    for label in models:
        ...
        if "catboost.core" in str(mdl.__class__):
            ...
            feature_importance = mdl.feature_importances_
        elif "PyTorchLSTMModel" in str(mdl.__class__):  # ✅ Pytorch Support
            with torch.no_grad():
                feature_importance = mdl.lstm_layers[0].weight_ih_l0.abs().sum(dim=0).cpu().numpy()
        else:
            logger.info("Model type does not support generating feature importances.")
            return

        # ✅ Ensure feature importance is normalized
        ...
        fig.update_layout(title_text=f"Best and Worst Features by Importance for {pair}")

        # ✅ Store Plot
        label = label.replace("&", "").replace("%", "")  # Escape FreqAI specific characters
        store_plot_file(fig, f"{dk.model_filename}-{label}.html", dk.data_path)

        logger.info(f"✅ Feature importance plot saved for {pair}")
